...
class WorkshopLocationWiseReport : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_true_value_location_wise)
        ...
        val typeOptions = mutableListOf("SELECT VEHICLE TYPE","ALL","SERVICE","DP")
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                jsonData?.let {
                    ...
                    runOnUiThread {
                        val adapter = ArrayAdapter(this@WorkshopLocationWiseReport, android.R.layout.simple_spinner_item, cities
                        )
    ...
    private fun fetchCountWise() {
        ...
        if (selectedType == "SELECT VEHICLE TYPE") {
            Toast.makeText(
                this@WorkshopLocationWiseReport,
                "Select vehicle model to proceed!",
                Toast.LENGTH_SHORT
            ).show()
        } else {
            val url = if (deptName == "SUPERADMIN") {
                ApiFile.APP_URL + "/srAccounts/srReportLocationCount?ouId=$selectedCity&dept=$selectedType"
            } else {
                ApiFile.APP_URL + "/srAccounts/srReportLocationCount?ouId=$ouId&dept=$selectedType"
            }

            val url2 = if (deptName == "SUPERADMIN") {
                ApiFile.APP_URL + "/srAccounts/srReportLocationCountSum?ouId=$selectedCity&dept=$selectedType"
            } else {
                ApiFile.APP_URL + "/srAccounts/srReportLocationCountSum?ouId=$ouId&dept=$selectedType"
            }
//        val url = ApiFile.APP_URL + "/accounts/reportLocModelCount?ouId=$ouId"
//        val url2 = ApiFile.APP_URL + "/accounts/reportAllModelCount?ouId=$ouId"
                    ...
                    jsonData?.let {
                        ...
                        for (i in 0 until jsonArray.length()) {
                            ...
                            val data = listOf(
                                ...
                                stockItem.getInt("DELIVERED").toString(),
                                stockItem.getInt("TOTALSTOCK").toString(),
                                stockItem.getInt("UNINVOICE").toString(),
                                stockItem.getInt("INVOICE").toString(),
                                stockItem.getInt("READYFORDELIVERED").toString(),
