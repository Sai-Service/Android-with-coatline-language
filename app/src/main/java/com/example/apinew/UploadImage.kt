...
//    private var vinNumber: String? = null
...
//                                Toast.makeText(this@UploadImage, "5 Images already present for this vehicle!", Toast.LENGTH_LONG).show()
//                                val intent = Intent(this@UploadImage, ChassisActivity::class.java)
//                                intent.putExtra("VIN", chassisNo)
...
class UploadImage : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        vinNumber = intent.getStringExtra("VIN")
        chassisNo=intent.getStringExtra("CHASSIS_NO")
        chassisTextView.text = vinNumber
        ...
        btnUpload.setOnClickListener { photoFile?.let { file -> chassisNo?.let { chassisNo ->
            vinNumber?.let { vinNumber ->
                uploadImage(file, chassisNo,vinNumber)
            }
        }
        }
        }
    ...
    private fun uploadImage(file: File, chassisNo: String,vinNumber:String) {
        val client = OkHttpClient()

        val requestBody = MultipartBody.Builder()
            .setType(MultipartBody.FORM)
            .addFormDataPart("file", file.name, RequestBody.create("image/jpeg".toMediaTypeOrNull(), file))
            .addFormDataPart("vin", vinNumber)
            .addFormDataPart("chassisNo", chassisNo)
            .addFormDataPart("lastUploadedBy", login_name)
            .build()
            ...
            try {
                ...
                runOnUiThread {
                    if (responseBody != null) {
                        ...
                        when {
                            message.contains("Five Images Already Uploaded For $vinNumber", ignoreCase = true) -> {
                                Toast.makeText(this@UploadImage, "5 Images already present for this vehicle!", Toast.LENGTH_LONG).show()
                                val intent = Intent(this@UploadImage, ChassisActivity::class.java)
                                intent.putExtra("VIN", chassisNo)
                                intent.putExtra("login_name", login_name)
