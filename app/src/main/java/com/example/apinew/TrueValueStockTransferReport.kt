...
class TrueValueStockTransferReport : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_true_value_stock_transfer)
                ...
                jsonData?.let {
                    ...
                    runOnUiThread {
                        val adapter = ArrayAdapter(
                            this@TrueValueStockTransferReport,
                            android.R.layout.simple_spinner_item,
                            cities
                        )
    ...
    private fun fetchUninvoice() {
        ...
        if (selectedType == "SELECT VEHICLE TYPE") {
            ...
            val url = if (deptName == "TVSUPERADMIN") {
                ApiFile.APP_URL + "/tvReports/tvStkTrfDetailsByOu?ou=$selectedCityCode&vehType=$selectedType"
            } else {
                ApiFile.APP_URL + "/tvReports/tvStkTrfDetailsByOu?ou=$ouId&vehType=$selectedType"
            }
                        ...
                        if (jsonObject.has("obj")) {
                            ...
                            for (i in 0 until jsonArray.length()) {
                                ...
                                val data = listOf(
                                    ...
                                    stockItem.optString("STOCK_TRF_NO", ""),
                                    formatDateTime(stockItem.optString("STOCK_TRF_DATE", "")),
                                    stockItem.optString("MODEL_DESC", ""),
                                    stockItem.optString("VARIANT_CODE", ""),
                                    stockItem.optString("VEH_STATUS", ""),
                                    stockItem.optString("STATUS", ""),
                                    stockItem.getInt("FROMKM").toString(),
                                    stockItem.getInt("TOKM").toString(),
                                    stockItem.optString("TO_LOCATION", ""),
                                    formatDateTime(stockItem.optString("FROM_LOCATION", "")),
                                    stockItem.optString("TRANSFERRED_BY", ""),
