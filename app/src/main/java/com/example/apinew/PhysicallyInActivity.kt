...
class PhysicallyInActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_physically_in)
    ...
    private fun fetchUninvoice() {
        ...
        if (selectedType == "SELECT VEHICLE TYPE") {
            ...
            val url = if (deptName == "SUPERADMIN") {
                ApiFile.APP_URL + "/accounts/reportPhysicallyIn?ouId=$selectedCityCode&vehType=$selectedType"
            } else {
                ApiFile.APP_URL + "/accounts/reportPhysicallyIn?ouId=$ouId&vehType=$selectedType"
            }

            val todayDate = SimpleDateFormat("dd-MMM-yyyy", Locale.getDefault()).format(Date())
//        val url = ApiFile.APP_URL + "/accounts/reportPhysicallyIn?ouId=$ouId"
            val request = Request.Builder()
                .url(url)
                .get()
                        ...
                        if (jsonObject.has("obj")) {
                            ...
                            for (i in 0 until jsonArray.length()) {
                                ...
                                val data = listOf(
                                    ...
                                    stockItem.optString("ACCOUNT_NUM", ""),
                                    ...
                                    stockItem.optString("SOB_NO", ""),
                                    stockItem.optInt("GATE_PASS_ID").toString(),
                                    stockItem.optString("DATE_OF_DELIVERY", ""),
                                    stockItem.optString("GATEPASSLOCATION", ""),
                                    stockItem.optString("VEHSTATUS", ""),
...
//                                stockItem.optInt("COUNT", 0).toString()
                                )
    ...
    private fun updateTableView(stockItems: List<List<String>>) {
        ...
        val headers = listOf(
            ...
            "VARIANT DESC",
            "COLOUR",
            "DMSINV NO","SOB NO","GATEPASS ID","DATE OF DELIVERY","GATEPASS LOCATION","VEHICLE STATUS","STATUS",
            "LOCATION"
        )
