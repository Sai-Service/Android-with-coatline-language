...
class TrueValueUploadImage : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_true_value_upload_img)
        ...
        chassisNo = intent.getStringExtra("REG_NO")
    ...
    private fun uploadImage(file: File, chassisNo: String) {
        ...
        val request = Request.Builder()
            .url("${ApiFile.APP_URL}/tvImage/tvUpload")
            .post(requestBody)
            .build()
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                    ...
                    if (responseBody != null) {
                        ...
                        when {
                            message.contains("Five Images Already Uploaded For $chassisNo", ignoreCase = true) -> {
                                Toast.makeText(this@TrueValueUploadImage, "5 Images already present for this vehicle!", Toast.LENGTH_LONG).show()
                                val intent = Intent(this@TrueValueUploadImage, TrueValVehicleEnquiry::class.java)
                                intent.putExtra("REG_NO", chassisNo)
                            ...
                            responseCode == 200 -> {
                                Toast.makeText(this@TrueValueUploadImage, "Image uploaded successfully", Toast.LENGTH_LONG).show()
                                val intent = Intent(this@TrueValueUploadImage, TrueValVehicleEnquiry::class.java)
                                intent.putExtra("REG_NO", chassisNo)
                                intent.putExtra("login_name", login_name)
                                progressBar.visibility=View.GONE
                                ...
                                progressBar.visibility=View.GONE
                                uploadingTextView.visibility=View.GONE
                                Toast.makeText(this@TrueValueUploadImage, "Something went wrong!\nPlease try again..", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        progressBar.visibility=View.GONE
                        uploadingTextView.visibility=View.GONE
                        Toast.makeText(this@TrueValueUploadImage, "No response from server", Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
                ...
                runOnUiThread {
                    progressBar.visibility=View.GONE
                    uploadingTextView.visibility=View.GONE
                    Toast.makeText(this@TrueValueUploadImage, "Error saving image: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
