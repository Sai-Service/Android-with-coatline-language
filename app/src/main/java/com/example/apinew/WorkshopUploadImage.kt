...
class WorkshopUploadImage : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_workshop_upload_image)
        ...
        chassisNo = intent.getStringExtra("REGNO")
    ...
    private fun uploadImage(file: File, chassisNo: String) {
        ...
        val requestBody = MultipartBody.Builder()
            ...
            .addFormDataPart("regNo", chassisNo)
            .addFormDataPart("lastUploadedBy", login_name)
            .build()

        val request = Request.Builder()
            .url("${ApiFile.APP_URL}/servImage/srUpload")
            .post(requestBody)
            .build()
            ...
            try {
                ...
                runOnUiThread {
                    if (responseBody != null) {
                        ...
                        when {
                            message.contains("Five images already uploaded for $chassisNo", ignoreCase = true) -> {
                                Toast.makeText(this@WorkshopUploadImage, "5 images already present for this vehicle!", Toast.LENGTH_LONG).show()
                                val intent = Intent(this@WorkshopUploadImage, WorkShopChassisEnquiry::class.java)
                                intent.putExtra("REGNO", chassisNo)
                            ...
                            responseCode == 200 -> {
                                Toast.makeText(this@WorkshopUploadImage, "Image uploaded successfully", Toast.LENGTH_LONG).show()
                                val intent = Intent(this@WorkshopUploadImage, WorkShopChassisEnquiry::class.java)
                                intent.putExtra("REGNO", chassisNo)
                                intent.putExtra("login_name", login_name)
                                progressBar.visibility=View.GONE
                                ...
                                progressBar.visibility=View.GONE
                                uploadingTextView.visibility=View.GONE
                                Toast.makeText(this@WorkshopUploadImage, "Failed to upload image. Error code: $responseCode", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        progressBar.visibility=View.GONE
                        uploadingTextView.visibility=View.GONE
                        Toast.makeText(this@WorkshopUploadImage, "No response from server", Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
                runOnUiThread {
                    progressBar.visibility=View.GONE
                    uploadingTextView.visibility=View.GONE
                    Toast.makeText(this@WorkshopUploadImage, "Error saving image: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
