...
class Forgotp : AppCompatActivity() {
    ...
    private fun updatePassword() {
        ...
        val oldPassword = npasswordEditText.text.toString().trim()
        val nPassword = cPasswordEditText.text.toString().trim()

        if (loginName.isEmpty() || oldPassword.isEmpty() || nPassword.isEmpty()) {
            Toast.makeText(this, "All fields are required", Toast.LENGTH_SHORT).show()
            return
        }

        if (oldPassword == nPassword) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                e.printStackTrace()
                runOnUiThread {
                    Toast.makeText(this@Forgotp, "Failed to update password", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                runOnUiThread {
                    try {
                        ...
                        Log.e("Forgotp", "Error parsing response: $responseBody", e)
                        Toast.makeText(
                            this@Forgotp,
